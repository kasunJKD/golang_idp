...
func (c DBConfig) MigrateAccount(req *pb.Request) (*pb.Response, error) {
	//Inserting new User
	c.log.Infolog.Println("Migrating to p99 account")
	sqlStatement := `
	WITH inUser AS (
		INSERT INTO users(userId, email, emailVerified, createdAt, updatedAt, passwordHash)
		VALUES ($14, $1, $2, (select current_timestamp at time zone ('utc')), (select current_timestamp at time zone ('utc')), $3)
	...
	inserterror := c.rwDb.QueryRow(sqlStatement,
		...
		req.GetExperience(),
		req.GetPlayingTime(),
		req.GetPreferredPlatforms(),
		req.GetLinkedUserId()).Scan(&userId, &createdAt, &updatedAt)
